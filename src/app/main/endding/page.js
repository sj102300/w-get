...
function EnterResult() {
    ...
    let [regular, setRegular] = useState(0);
    let [plasticBag, setPlasticBag] = useState(0);
    let [can, setCan] = useState(0);
    let [plastic, setPlastic] = useState(0);
        ...
        <div>
            ...
            <div className='enter-result'>
                ...
                <div className='trash-container'>
                    <TrashInput name={"일반쓰레기"} num={regular} setNum={setRegular} />
                    <TrashInput name={"비닐"} num={plasticBag} setNum={setPlasticBag} />
                    <TrashInput name={"캔"} num={can} setNum={setCan} />
                    <TrashInput name={"플라스틱"} num={plastic} setNum={setPlastic} />
                    ...
                    <div className='btn-container'>
                        <button className='white-btn w-full'>건너뛰기</button>
                        <button className='green-btn w-full'
                            onClick={() => {
                                ...
                                if (!accessToken) {
                                    ...
                                    fetch('/api/plogging-log', {
                                        method: "POST",
                                        headers: {
                                            Authorization: `Bearer ${accessToken}`
                                        },
                                        body: JSON.stringify({
                                            accumulated_time: Number(searchParams.get('time')),
                                            regular: Number(regular),
                                            plastic_bag: Number(plasticBag),
                                            can: Number(can),
...
function TrashInput({ name, num, setNum }) {
